...
- `bid >= ask`（クロス市場）の行はスキップ
- タイムスタンプが単調増加でない行はスキップ
- デフォルト通貨ペア: `USD/JPY`
...
- **パフォーマンス監視のテスト**: トラッカー、アラート、レポートが正しく生成されるか
- **構造的安全性の保証**: **フォワードテストは実際のオーダーパスに決して接続されない**。これは構造的に保証されており、バグによっても実際の注文が発注されることはない

フォワードテストクレート（`fx-forward`）も**ライブラリのみ**であり、CLI引数解析は持たない。プログラム的に使用される。
...
let mut runner = ForwardTestRunner::new(config, feed)?;

// 実行（シード指定で決定的再現）
let result = runner.run(12345)?;
...
| `format` | `Both` | 出力形式: `Json`, `Csv`, `Both` |
| `interval` | `None` | 定期レポート生成間隔（未実装） |
